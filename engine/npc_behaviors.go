...
func (app *App) setNPCInside(ctx context.Context, npcID string, inside bool, structureID string) {
	...
	// On entry we drop the actor in the structure's 'common' room; on
	// exit we clear. /pc/move-room handles the subsequent common→private
...
func (app *App) refreshStructureOccupancyState(ctx context.Context, structureID string) {
	...
	// canEnter / isBusinessClosed predicate already gates door entry on
	// these windowed states, but the visual current_state was driven
	...
	// here so the visual flips in lockstep with the predicate.
	var occupants int
	if err := app.DB.QueryRow(ctx,
