...
// bush renders is DERIVED from the object's finite gatherable supply — the same
// tag-driven model the day/night phase flip (world_phase.go) and structure
// occupancy (occupancy.go) use.
//
// Derived flag:
//
//	berries = (any finite gatherable refresh row on the object has stock left)
//
...
// finite gatherable supply and applies it if it changed. No-op unless the
// object's asset carries BOTH a 'berries'- and a 'bare'-tagged state —
// otherwise there's no defined pair to toggle between, so the object simply
...
func refreshObjectBerryState(w *World, obj *VillageObject) {
	...
	// "Berries" reflects pickable stock: any finite GATHERABLE row with units
	// left. Narrower than "any finite row" on purpose — a future bush with a
	// second, non-berry finite supply (e.g. sap) must not falsely keep berries
	// showing. IsFinite guarantees AvailableQuantity != nil, so the deref is safe.
	hasStock := false
