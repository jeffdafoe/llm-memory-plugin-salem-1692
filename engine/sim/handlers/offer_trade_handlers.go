...
func DecodeOfferTradeArgs(raw json.RawMessage) (any, error) {
	...
	if len(trimmed) == 0 || trimmed[0] != '{' {
		return nil, errors.New("offer_trade: arguments must be a JSON object")
	}
	...
	if err := dec.Decode(&extra); err != io.EOF {
		if err == nil {
			return nil, errors.New("offer_trade: trailing data after JSON object")
		}
		return nil, fmt.Errorf("offer_trade: malformed trailing data: %w", err)
	}

	if args.With == "" {
		return nil, errors.New("offer_trade: with is required")
	}
	...
	if args.WantItem == "" {
		return nil, errors.New("offer_trade: want_item is required")
	}
	...
	if args.WantQty < 1 {
		return nil, fmt.Errorf("offer_trade: want_qty must be at least 1 (got %d)", args.WantQty)
	}
	if args.WantQty > sim.MaxPayWithItemQty {
		return nil, fmt.Errorf("offer_trade: want_qty exceeds maximum (got %d, max %d)", args.WantQty, sim.MaxPayWithItemQty)
	}
	...
	if args.Coins < 0 {
		return nil, fmt.Errorf("offer_trade: coins cannot be negative (got %d)", args.Coins)
	}
	if args.Coins > sim.MaxPayWithItemAmount {
		return nil, fmt.Errorf("offer_trade: coins exceeds maximum (got %d, max %d)", args.Coins, sim.MaxPayWithItemAmount)
	}
	...
	if args.Coins == 0 && len(args.Give) == 0 {
		return nil, errors.New("offer_trade: trade must give goods or coins (add give lines, set coins, or both)")
	}
...
func validateGiveItemsDecode(items []payItemArg) error {
	...
	for i, pi := range items {
		...
		if pi.Qty < 1 {
			return fmt.Errorf("offer_trade: give[%d].qty must be at least 1 (got %d)", i, pi.Qty)
		}
		if pi.Qty > sim.MaxPayWithItemQty {
			return fmt.Errorf("offer_trade: give[%d].qty exceeds maximum (got %d, max %d)", i, pi.Qty, sim.MaxPayWithItemQty)
		}
	}
	return nil
}
