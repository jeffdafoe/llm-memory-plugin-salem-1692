...
// the situation that renders the "## Your stall" cue (payload.StallRepair non-nil:
// the owner stands at their own worn stall), and nowhere else. Same discussion-109
// "advertise a tool only with its triggering perception" invariant as craft/gather.
func TestGateTools_RepairOnlyWithStallCue(t *testing.T) {
	...
	if none["repair"] != 0 {
		t.Errorf("repair advertised with no '## Your stall' cue (count %d)", none["repair"])
	}

	// At the owner's own worn stall (StallRepair present) → repair is advertised once.
	at := specNameSet(gateTools(r, perception.Payload{
		ActorID:     "ezekiel",
		StallRepair: &perception.StallRepairView{NailsNeeded: 5, NailsHeld: 2},
	}, nil))
	if at["repair"] != 1 {
		t.Errorf("repair not advertised at the owner's worn stall (count %d)", at["repair"])
	}
}
