...
func DecodeDeliverOrderArgs(raw json.RawMessage) (any, error) {
	...
	if len(trimmed) == 0 || trimmed[0] != '{' {
		return nil, decodeErrf("deliver_order: arguments must be a JSON object")
	}
	...
	if err := dec.Decode(&extra); err != io.EOF {
		if err == nil {
			return nil, decodeErrf("deliver_order: trailing data after JSON object")
		}
		return nil, fmt.Errorf("deliver_order: malformed trailing data: %w", err)
	}
	if args.OrderID < 1 {
		return nil, decodeErrf("deliver_order: order_id must be at least 1 (got %d)", args.OrderID)
	}
	return args, nil
}
