...
func TestSubscriber_LaborOfferReceived_StampsEmployer(t *testing.T) {
	...
	solicitWork(t, w, "anne", "Prudence", 5, 240)
	...
	if reason.Worker != "anne" || reason.Reward != 5 || reason.DurationMin != 240 {
		t.Errorf("reason = %+v, want worker anne / reward 5 / duration 240", reason)
	}
	if reason.LaborID == 0 {
...
func TestSubscriber_LaborOfferReceived_SkipsPCEmployer(t *testing.T) {
	...
	defer stop()
	armLaborWorker(t, w, "anne", 1)
	solicitWork(t, w, "anne", "Patron", 5, 240)
