...
func TestDecodeSolicitWorkArgs_RewardItems(t *testing.T) {
	...
	// Coins + goods.
	res, err := decode(`{"employer":"Hannah","reward":2,"reward_items":[{"item":"porridge","qty":1}],"duration_minutes":120}`)
	if err != nil {
	...
	if _, err := decode(`{"employer":"Hannah","reward":0,"reward_items":[{"item":"porridge","qty":1}],"duration_minutes":120}`); err != nil {
		t.Errorf("goods-only reward should decode: %v", err)
	}

	// The stringified-array weak-model tolerance rides payItemList.
	if res, err := decode(`{"employer":"Hannah","reward":0,"reward_items":"[{\"item\":\"porridge\",\"qty\":1}]","duration_minutes":120}`); err != nil {
	...
	if _, err := decode(`{"employer":"Hannah","reward":0,"duration_minutes":120}`); err == nil {
		t.Error("0-coin, no-goods reward should be rejected")
	}
	// Negative coins.
	if _, err := decode(`{"employer":"Hannah","reward":-1,"reward_items":[{"item":"porridge","qty":1}],"duration_minutes":120}`); err == nil {
		t.Error("negative reward should be rejected")
	}
	// Per-line qty floor rides validatePayItemsDecode.
	if _, err := decode(`{"employer":"Hannah","reward":0,"reward_items":[{"item":"porridge","qty":0}],"duration_minutes":120}`); err == nil {
	...
	if _, err := decode(`{"employer":"Hannah","reward":0,"reward_items":[` + lines + `],"duration_minutes":120}`); err == nil {
