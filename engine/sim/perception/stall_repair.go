...
// stall_repair.go — LLM-118 owner audience. The "## Your stall" section: a
// standing reminder, shown whenever a stall owner stands at their own worn market
// stall, that it needs mending and how. A nil view renders nothing (the common
// case — not at the stall, or it isn't worn yet). The SAME non-nil view gates the
// `repair` tool's advertisement (handlers/tool_gating.go), so the cue and the
...
// StallRepairView is the owner's at-the-stall repair cue. Non-nil only when the
// actor owns the market stall they are standing at AND it has worn to the repair
// threshold.
...
// buildStallRepair returns the owner's at-the-stall repair cue, or nil. Pure over
// the snapshot. Gated on: the actor owns a wearable market stall, is standing at
...
// renderStallRepair writes the "## Your stall" section. Content-gated: a nil view
...
func renderStallRepair(b *strings.Builder, v *StallRepairView) {
	...
	b.WriteString("## Your stall\n")
	if v.Degraded {
		b.WriteString("Your market stall is too worn to trade — it stays shut until you mend it. ")
	} else {
		b.WriteString("Your market stall is showing hard use — its boards are loose. ")
	}
	if v.HasEnoughNails {
		fmt.Fprintf(b, "You carry enough nails (%d) to mend it — repair it now (it takes a short while, hammer in hand, at the stall).\n", v.NailsHeld)
	} else {
...
// owner — standing at a worn market stall. Social texture a passerby can remark
// on (and the perception twin of the worn sprite, which the art pass defers).
// nil when the actor isn't at a worn stall, or when they ARE its owner (they get
// the richer "## Your stall" cue instead).
type StallConditionView struct {
	Degraded bool
}

// buildStallCondition returns the co-present worn-stall line for a non-owner, or
// nil. Pure over the snapshot. Surfaces the FIRST worn stall the actor stands at
...
func buildStallCondition(snap *sim.Snapshot, actorID sim.ActorID, actorSnap *sim.ActorSnapshot) *StallConditionView {
	...
	for _, obj := range snap.VillageObjects {
		if !sim.IsWearableStall(obj) || obj.OwnerActorID == actorID {
			continue // non-stall/unowned, or my own (## Your stall covers me)
		}
		...
		return &StallConditionView{Degraded: sim.StallDegraded(obj, snap.StallWearDegradeThreshold)}
	}
	return nil
}

// renderStallCondition writes the co-present worn-stall atmosphere line. A nil
...
func renderStallCondition(b *strings.Builder, v *StallConditionView) {
	...
	if v.Degraded {
		b.WriteString("The market stall here is battered and clearly unfit for trade — its boards hang loose.\n")
	} else {
		b.WriteString("The market stall here looks worn, its boards loosened by hard use.\n")
	}
}
