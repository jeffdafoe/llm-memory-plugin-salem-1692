...
func Build(snap *sim.Snapshot, actorID sim.ActorID, warrants []sim.WarrantMeta, opts ...BuildOption) Payload {
	...
	//
	// LLM-168: gated on WORKLESS (no work_structure_id), not broke (Coins==0). A
	// workless worker has no post to keep, so seeking odd jobs is its only on-shift
	// activity whether or not it is penniless; the brand-new Walker family idled all
	// shift holding a few coins because the old broke gate never surfaced this for
	// them. A worker with a workplace is steered to its post by the duty steer.
	//
