...
func TestLaborOffer_WakesEmployerAndSurfacesOffer(t *testing.T) {
	...
	res, err := w.Send(sim.SolicitWork("anne", "Prudence Ward", 5, nil, 240, time.Now().UTC()))
	if err != nil {
	...
	if off.Worker != "anne" || off.Reward != 5 || off.DurationMin != 240 {
		t.Errorf("offer view = %+v, want worker anne / reward 5 / duration 240", off)
	}
}
...
func TestLaborOffer_UnaffordableEmployerAutoDeclinedNotWoken(t *testing.T) {
	...
	res, err := w.Send(sim.SolicitWork("lewis", "Prudence Ward", 5, nil, 240, time.Now().UTC()))
	if err != nil {
		t.Fatalf("SolicitWork: %v", err)
	}
