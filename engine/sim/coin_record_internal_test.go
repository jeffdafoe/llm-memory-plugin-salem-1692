...
func TestRecordCoinPaid_WritesBothDirections(t *testing.T) {
	...
	w.RecordCoinPaid("moses", "gideon", 1, at)
...
func TestRecordCoinPaid_IgnoresNonTransfers(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			w := newCoinWorld(0)
			w.RecordCoinPaid(tc.payer, tc.payee, tc.amount, at)
...
func TestRecordCoinPaid_PrunesToWindow(t *testing.T) {
	...
	w.RecordCoinPaid("moses", "gideon", 1, base.Add(-100*time.Hour)) // well outside
	w.RecordCoinPaid("moses", "gideon", 1, base.Add(-1*time.Hour))   // inside
	...
	w.RecordCoinPaid("moses", "gideon", 1, base.Add(-3*time.Hour))
	if len(rec.Paid) != 2 {
...
func TestRecordCoinPaid_CapCountsEvictions(t *testing.T) {
	...
	for i := 0; i < MaxCoinPaymentsPerPairDirection+extra; i++ {
		w.RecordCoinPaid("moses", "gideon", 1, base.Add(time.Duration(i)*time.Minute))
	}
	rec := w.CoinRecord["moses"]["gideon"]
...
func TestCloneCoinRecord_IsDeep(t *testing.T) {
	...
	w.RecordCoinPaid("moses", "gideon", 5, at.Add(time.Minute))
...
func TestRepublishCarriesTheCoinRecord(t *testing.T) {
	w := &World{}
	w.Actors = map[ActorID]*Actor{}
	w.RecordCoinPaid("moses", "gideon", 1, time.Now().Add(-time.Hour))
	w.republish()
	snap := w.Published()
	if snap == nil {
