...
// than waiting on the 15s production cadence: probeWorldCommand is the unit that
...
// The prober beats BEFORE it sends, which is the property that keeps the two
// instruments independent: a wedged world must not be able to silence the ticker
// that proves the prober is alive. If this inverts, ticker_stale and
// world_command_stalled collapse into the same silence and the ticket's whole
// premise is gone.
func TestRunWorldCommandProbe_BeatsEvenWhileTheWorldIsWedged(t *testing.T) {
	w := NewWorld(Repository{})
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	go w.Run(ctx)
	...
	// Beat the way the prober does, then probe. The beat must land regardless of
	// what the (wedged) world does with the command.
