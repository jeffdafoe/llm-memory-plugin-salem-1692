...
//
// Ambient scenery is excluded alongside PCs (LLM-593): the roster tells the
// atmosphere model who is about, and a duck listed by name reads as a
// resident — the eight pond ducks, all called "Duck" since LLM-586, filled
// the outdoor bucket with one repeated name. The exclusion is BehaviorAmbient
// and not the wider KindDecorative on purpose: the lamplighter, washerwoman
// and town crier are decorative carriers who really are about the village,
// and dropping them would thin the roster the mood prose is drawn from.
func buildVillageContextRoster(w *World) []AtmosphereRosterEntry {
	byLoc := make(map[string][]string)
	var outdoor []string
	for _, a := range w.Actors {
		if a == nil || a.Kind == KindPC || actorIsAmbient(w, a) {
			continue
		}
...
//
// The ambient arm of the actor filter is belt-and-braces (LLM-593): the
// append funnel keeps scenery rows out of ActionLog in the first place, so
// this can only matter for a log carried across the fix. It stays because the
...
func buildVillageContextActivityDigest(w *World) []ActivityDigestEntry {
	...
	for _, e := range w.ActionLog {
		...
		if !ok || a == nil || a.Kind == KindPC || actorIsAmbient(w, a) {
			continue
		}
