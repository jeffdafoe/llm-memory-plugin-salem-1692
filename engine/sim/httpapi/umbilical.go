...
func (s *Server) umbilicalRoutes() []umbilicalRoute {
	return []umbilicalRoute{
		...
		{http.MethodGet, umbilicalBasePath + "/settings", "Live operator-tunable world settings: per-need red-line thresholds (read side of settings/need-threshold; ephemeral) + the huddle loop-sweep knobs (read side of settings/huddle-loop; persisted) with an enabled flag + the seek-work coin ceiling (read side of settings/seek-work-ceiling; persisted) + the farm wealth-tax knobs (read side of farm-upkeep/set; persisted) + the labor produce boost (read side of settings/labor-produce-boost; persisted) + the merchant working-capital floor (read side of settings/merchant-coin-floor; persisted) + the stall wear & repair knobs including the degraded-production pct (read side of stall-wear/set; persisted) + the eco-mode knobs with live audience/engaged state (read side of settings/eco-mode; persisted).", false, s.handleUmbilicalSettings},
		...
		{http.MethodPost, umbilicalBasePath + "/settings/eco-mode", "Live-tune eco mode (LLM-313): while no player character has a fresh presence stamp, the reactor paces social-bucket warrant cycles (npc_spoke, huddle beats) by social_gap_seconds and economy-bucket cycles (restock, production choice, farm upkeep, stall repair, seek work) by economy_gap_seconds; survival/duty/commerce warrants are never slowed. The plain idle backstop and visitor spawning also pause while unwatched. PERSISTED across restart. All fields optional, at least one required. Body: {enabled?, social_gap_seconds?, economy_gap_seconds?} — gaps >= 0 (0 disables that throttle) and strictly below the warrant stale horizon (MaxWarrantAge, default 90s) so a parked cycle always comes due before it can age out.", true, s.handleUmbilicalEcoMode},
