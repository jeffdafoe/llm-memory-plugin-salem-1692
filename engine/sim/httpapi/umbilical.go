...
func (s *Server) umbilicalRoutes() []umbilicalRoute {
	return []umbilicalRoute{
		...
		{http.MethodGet, umbilicalBasePath + "/settings", "Live operator-tunable world settings: per-need red-line thresholds (read side of settings/need-threshold; ephemeral) + the huddle loop-sweep knobs (read side of settings/huddle-loop; persisted) with an enabled flag.", false, s.handleUmbilicalSettings},
