...
func (s *Server) umbilicalRoutes() []umbilicalRoute {
	return []umbilicalRoute{
		...
		{http.MethodPost, umbilicalBasePath + "/worker/provision", "Mint an actor into a live Worker: assign a backing VA (default salem-vendor), grant the `worker` attribute, and reclassify its Kind in memory so a sprite-only decorative comes online and takes solicit_work jobs WITHOUT a restart (the checkpoint persists the link+attribute for the next reload). An unscheduled worker is then day-active on the world dawn/dusk window automatically (LLM-137). Body: {actor_id, agent?}.", true, s.handleUmbilicalProvisionWorker},
