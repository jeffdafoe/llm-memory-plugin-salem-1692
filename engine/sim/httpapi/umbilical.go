...
func (s *Server) umbilicalRoutes() []umbilicalRoute {
	return []umbilicalRoute{
		...
		{http.MethodGet, umbilicalBasePath + "/sell-through", "Per-(seller, item) recent sell-through off the published snapshot's price book — the demand signal the reseller restock cue reasons against: units sold, sale count, coins, distinct buyers over a trailing window, highest-throughput first. Query params: actor (filter to one seller), item, window_hours (default 168).", false, s.handleUmbilicalSellThrough},
