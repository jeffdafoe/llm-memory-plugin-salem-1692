...
func (s *Server) umbilicalRoutes() []umbilicalRoute {
	return []umbilicalRoute{
		...
		{http.MethodGet, umbilicalBasePath + "/objects", "Placed village objects off the published snapshot (read side of the object/* control routes): per object its asset, position (world-pixel + tile), display-name, state, owner, entry-policy, loiter-offset, tags, refresh-policy (including each row's live gatherable stock), attached-to, and whether it backs a structure. ALSO the mutable runtime counters, each omitted at zero: `wear` (accrued maintenance demand on an owned business) and `rate_owed` (unpaid town-rate arrears), both of which /settings reports the thresholds for but nothing else reads live, plus `hearth_lit_until` (when a hearth's fire burns out). Raw numbers only — no derived degraded/needs-repair flag; owner_actor_id + tags + the /settings thresholds are all the terms those judgments take. Query filters (all optional): id, owner, tag, structure — `?tag=business` narrows to the rateable/wearable businesses.", false, s.handleUmbilicalObjects},
		...
		{http.MethodGet, umbilicalBasePath + "/settings", "Live operator-tunable world settings: per-need red-line thresholds (read side of settings/need-threshold; ephemeral) + the huddle loop-sweep knobs (read side of settings/huddle-loop; persisted) with an enabled flag + the seek-work coin ceiling (read side of settings/seek-work-ceiling; persisted) + the farm wealth-tax knobs (read side of farm-upkeep/set; persisted) + the labor produce boost (read side of settings/labor-produce-boost; persisted) + the merchant working-capital floor (read side of settings/merchant-coin-floor; persisted) + the stall wear & repair knobs including the degraded-production pct (read side of stall-wear/set; persisted) + the eco-mode knobs with live audience/engaged state and the LLM-466 audience idle horizon (read side of settings/eco-mode; persisted).", false, s.handleUmbilicalSettings},
